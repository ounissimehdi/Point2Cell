...
Created on Mon Jan 27 14:34:22 2022
    Sorbonne Université, Sorbonne Center of Artificial Intelligence (SCAI)
    Paris Brain Institute -ICM- (INSERM, CNRS, Sorbonne Univeristé, AP-HP), INRIA "ARAMIS Lab"
...
if __name__ == '__main__':
    ...
    exp_name = ''
    if cross_val !=0:
        exp_name += 'norm_'+str(cross_val)
        exp_name += '_cross-val_'
    exp_name += 'density_mask_unet_'
    ...
    main_data_dir = os.path.join('..', '..', '..', 'dataset','augmented_hela_cells_dataset')
    ...
    train_img_paths_list = natsorted(glob(os.path.join(train_dir, '*_img.tif')))
    train_mask_paths_list = natsorted(glob(os.path.join(train_dir, '*_density_mask.tif')))

    # All image/mask validation paths
    val_img_paths_list = natsorted(glob(os.path.join(val_dir, '*_img.tif')))
    val_mask_paths_list = natsorted(glob(os.path.join(val_dir, '*_density_mask.tif')))

    # All image/mask validation paths
    test_img_paths_list = natsorted(glob(os.path.join(test_dir, '*_img.tif')))
    test_mask_paths_list = natsorted(glob(os.path.join(test_dir, '*_density_mask.tif')))

    # No cross validation is used
    if cross_val == 0:
        # Preparing the training dataloader
        train_dataset = CustomDataset(train_img_paths_list, train_mask_paths_list, ref_image_path, normalize=True,cached_data=pin_memory, n_channels=n_input_channels,scale=scale_factor)
        ...
        val_dataset = CustomDataset(val_img_paths_list, val_mask_paths_list, ref_image_path, normalize=True,cached_data=pin_memory, n_channels=n_input_channels,scale=scale_factor)
        ...
        # Preparing the validation dataloader
        test_dataset = CustomDataset(test_img_paths_list, test_mask_paths_list, ref_image_path, normalize=True,cached_data=pin_memory, n_channels=n_input_channels, scale=scale_factor)
    ...
    # Cross-validation will be used
        ...
        for i in range(cross_val):
            if not(folds_done[i]):
                ...
                # Preparing the training dataloader
                train_dataset = CustomDataset(fold_train_imgs_paths, fold_train_masks_paths, ref_image_path, normalize=True,cached_data=pin_memory, n_channels=n_input_channels,scale=scale_factor)
                train_loader = DataLoader(train_dataset, batch_size=batch_size*n_devices, shuffle=True, pin_memory=pin_memory, num_workers=n_workers)

                # Preparing the validation dataloader
                val_dataset = CustomDataset(fold_val_imgs_paths, fold_val_masks_paths, ref_image_path, normalize=True,cached_data=pin_memory, n_channels=n_input_channels,scale=scale_factor)
                val_loader = DataLoader(val_dataset, batch_size=1*n_devices, shuffle=False, pin_memory=pin_memory, num_workers=n_workers)
                ...
                logging.info(f'''
                        ...
                        Test loss per image :  {test_loss/len(test_loader)}
                        Dice                :  {total_dice_coeff}
