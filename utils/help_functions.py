...
class CustomDataset(Dataset):
    ...
    def __init__(self, imgs_dirs, masks_dirs, ref_image_path, normalize=False,cached_data=True, n_channels=1,scale=1):
        ...
        # Make sure the scale is bewteen [0, 1]
        assert 0 < scale <= 1, '[ERROR] Scale must be between 0 and 1'

        # Load the refenece image into RAM
        ref_image = Image.open(ref_image_path)
        
        # Save the refrenece image into RAM to be used
        self.ref_image = ref_image.copy()

        # Caching the dataset (WARRNING : this needs to be used when you have big RAM memory)
        if cached_data:
    ...
    def preprocess(self, pil_img, ref_image, n_channels, scale, normalize, mask=True):
        if not(mask):
            ...
            if normalize:
                # Make sure the refrenece image and the current image have the same size
